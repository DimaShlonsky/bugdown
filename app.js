...
var serverUrl,
    ...
    bugN,
    bugUrl,
    //sessionId = "69a00b45-839f-45f1-a76f-6d18d6f0fc74",
    resolutionText,
    resolvedInRev,
    resolvedInText,
...
$ bugbot --bugNumber|-n <bug number> --resolutionMessage|-m <resolution message> --resolvedInRevision|-r <resolution revision> <params> [options]
OR
$ bugbot svn-hook <working copy dir> <params> [options]
OR
$ bugbot by-svn [options] <path> <depth> <messagefile> <revision> <error> <cwd>
...
baseURL = cli.flags.b || "http://mobile.toptix-online.com:8080/InformUP";
resolvedInText = "Rev. " + resolvedInRev;
bugUrl = baseURL + "/Form/Form.aspx?ItemType=ExistingItem&Item=" + bugN;
serverUrl = cli.flags.s || 'http://localhost:4444/wd/hub';
...
function login() {
    return browser.getCurrentUrl().then(function (url) {
        ...
        if (!url.pathname.toLowerCase().endsWith("login.aspx")) {
            return browser.get(baseURL + "/Login.aspx");
        }
    }).then(function () {
